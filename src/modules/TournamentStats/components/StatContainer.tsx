...
export const StatContainer = ({ stat }: { stat: StatType }) => {
  const { getStandings, getTop5CategoryPerformances, getTop5CategoryTotals } = useTable();
  const { characters } = useTournament();

  const topFive = getStandings(stat.id)?.slice(0, 5) || [];
  console.log(getTop5CategoryPerformances())
  console.log(getTop5CategoryTotals())
  return (
        ...
        {topFive.map((char, index) => {
          ...
          return (
            ...
            >
              ...
              <span className={`w-10 text-right  ${index === 0 ? "text-amber-400" : "text-gray-300"}`}>{char[stat.id]}</span>
            </div>
          );
        })}
      </div>
